...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <form action="{{ route('admin.schedules.update', $schedule->schedule_id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select name="course_id" id="course_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 @error('course_id') border-red-500 @enderror">
                        ...
                        @foreach($courses as $course)
                            <option value="{{ $course->course_id }}" {{ (old('course_id', $schedule->course_id) == $course->course_id) ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="instructor_id" id="instructor_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 @error('instructor_id') border-red-500 @enderror">
                        ...
                        @foreach($instructors as $instructor)
                            <option value="{{ $instructor->instructor_id }}" {{ (old('instructor_id', $schedule->instructor_id) == $instructor->instructor_id) ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="time" name="time_start" id="time_start" value="{{ old('time_start', \Carbon\Carbon::parse($schedule->time_start)->format('H:i')) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 @error('time_start') border-red-500 @enderror">
                    @error('time_start')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="time" name="time_end" id="time_end" value="{{ old('time_end', \Carbon\Carbon::parse($schedule->time_end)->format('H:i')) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 @error('time_end') border-red-500 @enderror">
                    @error('time_end')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mb-6">
                ...
                <input type="text" name="room" id="room" value="{{ old('room', $schedule->room) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 @error('room') border-red-500 @enderror" placeholder="e.g. Room 101">
                @error('room')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
